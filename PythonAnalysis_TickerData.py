...
def fetch_ticker_data(symbol):
    ...
    # Basic Requirement(Uncomment to Check)
    # bullish_ticker = (binance_data['SMA5'] > binance_data['SMA20'])
...
for ticker, data in ticker_data.items():
    ...
    percentage_difference = ((max_arima_forecast - current_close) / current_close) * 100
     ...
     # Check if the ticker is in the top bullish tickers
    if ticker in [t[0] for t in top_bullish_tickers]:
        percentage_difference = ((max_arima_forecast - current_close) / current_close) * 100
